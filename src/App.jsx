...
export default function App() {
  ...
  const exportData = () => {
    try {
      ...
      const totalProgress = days.reduce((sum, day) => sum + (day.progress || 0), 0);
      const avgProgress = (totalProgress / 7).toFixed(0);
      const daysWithJournal = days.filter((d) => d.journal && d.journal.trim()).length;
      const daysWithTasks = days.filter((d) => d.tasks && d.tasks.length > 0).length;
      ...
      days.forEach((day, index) => {
        ...
        if (day.tasks && day.tasks.length > 0) {
          ...
          day.tasks.forEach((task) => {
            ...
            const taskLines = doc.splitTextToSize(checkbox + " " + task.text, 150);
