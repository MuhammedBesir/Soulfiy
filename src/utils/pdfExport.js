...
export function exportToPDF(days, aiSuggestions) {
  try {
    ...
    doc.setFontSize(24);
    doc.setFont("helvetica", "bold");
    doc.text(String(completedDays), startX + boxWidth / 2, boxY + 15, {
      align: "center",
    });
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text("Tamamlanan Gün", startX + boxWidth / 2, boxY + 23, {
      align: "center",
    });
    ...
    doc.text(
      String(totalHours),
      startX + boxWidth + spacing + boxWidth / 2,
      boxY + 15,
      { align: "center" }
    );
    doc.setFontSize(9);
    ...
    doc.text(
      String(aiUsageCount),
      startX + (boxWidth + spacing) * 2 + boxWidth / 2,
      boxY + 15,
      { align: "center" }
    );
    doc.setFontSize(9);
    ...
    if (completionPercentage >= 80) motivationMsg = "Mükemmel bir hafta! 🎉";
      ...
      motivationMsg = "İyi iş çıkarıyorsun! 💪";
    else if (completionPercentage > 0)
      motivationMsg = "Devam et, başarırsın! 🌟";
    ...
    const stats = [
      ...
      {
        icon: "journal",
        label: "Günlük Yazılan",
        value: String(daysWithJournal),
        color: blue,
      },
      {
        icon: "ai",
